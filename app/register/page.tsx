...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase
      .from('profiles')
      .insert([{ id: userId, username, correo: email }]);

    if (insertError) {
      setMessage('Usuario creado pero no guardado: ' + insertError.message);
      return;
    }
    ...
    <div className="min-h-screen bg-black flex items-center justify-center">
      <div className="w-96 bg-zinc-900 p-10 rounded-lg flex flex-col gap-4">
        ...
        <p className="text-zinc-400 text-sm text-center">
          ¿Ya tienes cuenta?{' '}
          <button onClick={() => router.push('/login')} className="text-white underline">
            Inicia sesión aquí
          </button>
        </p>
        {message && <p className="text-center text-sm mt-2 text-zinc-300">{message}</p>}
      </div>
    </div>
  );
}